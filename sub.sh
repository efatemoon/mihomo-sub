...
output="mixed-port: ${MIHOMO_MIXED_PORT}
...
# 注意：curl 的输出直接捕获状态码
sub_response=$(curl -s --max-time 15 -w "%{http_code}" -o /tmp/mihomo_temp.yml "http://127.0.0.1:${SUBCONV_PORT}/sub?target=clash&url=${encoded_sub_url}${encoded_config_param}")
sub_exit_code=$?
