...
docker run -d \
  ...
  -p 7890:7890 \
  -p 9090:9090 \
  ghcr.io/efatemoon/mihomo-sub:latest
```
...
services:
  mihomo:
    image: ghcr.io/efatemoon/mihomo-sub:latest
...
| `subconv_port`      | Subconverter 内部监听端口（**请勿与 Mihomo 端口冲突**） | `25500`         | `25500`                              |
| `mihomo_mixed_port` | Mihomo 的 `mixed-port` 端口                             | `7890`          | `7890`                               |

> **端口隔离说明**：容器内 Subconverter 服务固定监听 `25500`（可通过 `subconv_port` 修改），Mihomo 监听 `7890`（可通过 `mihomo_mixed_port` 修改）。两者使用独立端口，避免启动时发生端口抢占。
